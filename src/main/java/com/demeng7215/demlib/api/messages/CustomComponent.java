...
public final class CustomComponent {
	...
	public CustomComponent onHover(HoverEvent.Action action, String text) {
		currentComponent.setHoverEvent(new HoverEvent(action, TextComponent.fromLegacyText(MessageUtils.colorize(text))));
		return this;
	}
	...
	public void send(Player... players) {
		...
		for (final Player p : players)
			BungeeChatProvider.sendComponent(p, mainComponent);
	}
...
class BungeeChatProvider {
	...
	static void sendComponent(Player p, Object comps) {
		...
		if (comps instanceof TextComponent)
			BungeeChatProvider.sendComponent(p, comps);
		else
			sendComponent(p, (BaseComponent[]) comps);
	}
